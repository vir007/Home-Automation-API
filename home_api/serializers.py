...
class LightsSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        if status is not None and status != instance.status:
            Log_lights.objects.create(lights=instance, oldVal=instance.status, newVal=status)
...
class RoomSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        if temp is not None and temp != instance.temp:
            Log_room_temp.objects.create(room=instance, oldVal=instance.temp, newVal=temp)
...
class HomeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Home
        fields = ['id', 'name', 'owner', 'thermostat_temp', 'thermostat_mode','room']
    ...
    def update(self, instance, validated_data):
        ...
        instance.thermostat_temp = validated_data.get("thermostat_temp", instance.thermostat_temp)
        instance.thermostat_mode = validated_data.get("thermostat_mode", instance.thermostat_mode)
        instance.save()
        return instance
